...
// }
//create new image
export async function createImage(image: Omit<IImage, "id">): Promise<IImage> {
    try {
...
export async function fetchImageById({ id }: { id: string }) {
  try {
    const response = await axios.get<IImage>(`http://localhost:3001/images/${id}`);
    return response.data;
  } catch (error) {
...
export const updateImage = async ({
  ...
  try {
    const response = await axios.put(`http://localhost:3001/images/${id}`, data);
    return response.data;
  } catch (error) {
